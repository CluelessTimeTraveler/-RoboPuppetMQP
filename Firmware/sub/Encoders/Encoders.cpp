...
uint16_t Encoders::getPositionSPI(uint8_t encoder, uint8_t resolution)
{
  ...
  Serial.println("spiWriteRead2 complete");
  currentPosition |= temp;      

  Serial.println("or complete");
...
void Encoders::update(){
   ...
   //set attemps counter at 0 so we can try again if we get bad position    
    ...
    encoderPosition = getPositionSPI(encoder1, RES12); 
    Serial.println("Got position SPI");
    ...
    {
      encoderPosition = getPositionSPI(encoder1, RES12); //try again
    }
